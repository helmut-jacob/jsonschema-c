...
static inline int
json_handle_pattern_keyword(char *pattern_value, char *instance_value){
    regex_t pattern;
        ...
        if(json_object_is_type((struct json_object*)value->v, json_type_array)){
            ...
            if(lh_table_lookup_entry(instance_table,(char *)value->k) != NULL){
                ...
                for(i=0 ; i< json_object_array_length(value->v); i++){
                    char *dependency = json_object_get_string(json_object_array_get_idx(value->v,i));
            ...
            lh_foreach(dependencies_table, dependency){
                ...
                if(key != NULL){
                    lh_foreach(json_object_get_object(dependency->v), tmp){
                        ...
                        if(json_object_object_get_ex(key->v,tmp->k, NULL) != 1){
                            json_object_object_add(key->v,tmp->k,tmp->v);
                        }
                    }
                }
                else
            ...
            lh_foreach(dependencies_table, dependency){
                ...
                if(key != NULL){
                    ...
                    struct lh_entry *head = lh_table_lookup_entry(json_object_get_object(key->v), json_object_get_object(dependency->v)->head->k);
